	    ...
	    char BasicAlgo)
{
    ...
    a*=2*a;
    s = 0;
    if (BasicAlgo) {/* Without fast SIL computation, use standard 2D kernel form */
    ...
    } else { /* With fast SIL computation, use separable kernel form */
...
#pragma omp parallel
    {
        ...
        /* MAIN LOOP : denoise pixel x=(x1,x2)
           The first loop follows x1 axis and is dispatched on
