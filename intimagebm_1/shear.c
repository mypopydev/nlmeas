		 ...
		 float translation)
{
   ...
   if (NULL == line_in
      ...
      error("fftw_malloc() failed in image_shear()");

   // Plans computation
   p_fwd_dct =
       fftw_plan_r2r_1d(n, line_in, dct, FFTW_REDFT10, FFTW_ESTIMATE);
   p_back_dct =
       fftw_plan_r2r_1d(n, dct, line_out_sym, FFTW_REDFT01, FFTW_ESTIMATE);
   p_back_dst =
       fftw_plan_r2r_1d(n, dst, line_out_antisym, FFTW_RODFT01,
			FFTW_ESTIMATE);
