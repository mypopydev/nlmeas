...
void help(char **v)
{
   fprintf(stderr, "Block matching stereo using integral image\n");
   fprintf(stderr,
...
[disparity_Right mincost_Right]\n", v[0]);
   fprintf(stderr, "   The SUBPIXEL environ. var. determines the subpixel \
precision (default 1).\n");
   fprintf(stderr, "   The SLICED environ. var. determines the width of \
...
{
   int patch_sz[2], hor_range[2], ver_range[2] = { 0, 0 };
   ...
   //               patch_sz, hor_range, ver_range, method);
   // parallel and subpixel
   stereoIntSliceSubpix(in1, in2, disp, cost, dispR, costR,
			patch_sz, hor_range, ver_range, method);


   // ATTENTION: hack to save only the horizontal displacement, vert. is 0
   disp->nch = 1;
   dispR->nch = 1;
